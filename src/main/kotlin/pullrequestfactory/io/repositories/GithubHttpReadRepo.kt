...
import pullrequestfactory.io.repositories.GithubHttpHeaderLinkPageParser.parsePages

class GithubHttpReadRepo(val httpClient: HttpClient, val ui: UI) {

    inline fun <reified T> getList(res: Response, url: String): List<T> {
        val pages = parsePages(res.headers["link"])
    ...
    fun handle(response: Response, url: String) = when (response.statusCode) {
