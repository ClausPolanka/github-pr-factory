...
object Programs {
    ...
    fun create_program_for(args: Array<String>): Program {
        ...
        val basicAuthTokenFromProps = userProps.get_github_basic_auth_token()
        val pa = ProgramArgs(args, basicAuthTokenFromProps)
        val repoUrl = baseUrl + repoPath
        val basicAuthTokenFromArgs = pa.get_github_basic_auth_token()
        return when {
            ...
            pa.has_open_command_in_interactive_mode() -> OpenPullRequestsProgramInteractiveMode(ui, repoUrl, basicAuthTokenFromProps)
            pa.has_open_command_with_optional_options() -> OpenPullRequestsProgramWithOptionalOptions(ui, pa, repoUrl, basicAuthTokenFromArgs)
            pa.has_open_command() -> OpenPullRequestsProgram(ui, pa, repoUrl, basicAuthTokenFromArgs)
            ...
            pa.has_close_command_in_interactive_mode() -> ClosePullRequestsProgramInteractiveMode(ui, repoUrl, basicAuthTokenFromProps)
            pa.has_close_command() -> ClosePullRequestsProgram(ui, pa, repoUrl, basicAuthTokenFromArgs)
